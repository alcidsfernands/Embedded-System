...
def insert_neumatico(PRESION, TEMP, REVOLUCIONES, ANGULOGIRO, POS):
	...
	try:
		cnx1 = mysql.connector.connect(user = "root", password = "Muaeci_2022", host = "10.172.119.11", database = "F1MON")
		cursor = cnx1.cursor()
		...
		print("No se ha podido insertar en la BBDD - NEUMATICO", error)
...
def insert_POS(GPS, DISTANCIA, CAMARA):
	try:
		...
		query = """INSERT INTO POS (ID_COCHE, GPS, DISTANCIA, CAMARA, TIME) VALUES (%s,pointfromtest(%s),%s,%s,%s)"""
		NOW = datetime.datetime.utcnow()
		...
		print("No se ha podido insertar en la BBDD - POS")
